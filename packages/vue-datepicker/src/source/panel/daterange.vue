...
export default {
  ...
  methods: {
    init (date, display) {
      let value = moment(new Date()).subtract(this.panelNum - 1, 'M'); let selectedDate = []
      this.hoverStartDate = []
      this.hoverEndDate = []
      this.arrDate = []
      this.isHover = false
    ...
    timeInit (isShow = true) {
      if (ishms(this.format)) {
        ...
        const len = selectedDate.length
        if (len) {
          const date = selectedDate[len - 1]
    ...
    radio (date) {
      if (this.arrDate.length === 0) {
        ...
        if (date.getTime() > this.selectedDate[0][0].getTime()) {
          ...
          const endDate = this.selectedDate[0][0]
          this.selectedDate = []
          this.hoverStartDate = []
    ...
    optionsClick (item, e) {
      ...
      const dateValue = type === 'text' ? [item.text] : value

      if (value.length === 2 && isValid(value[0]) && isValid(value[1])) {
        const startDate = moment(new Date(value[0])).subtract(this.panelNum - 2, 'M')
    ...
    getMultipleFn (fn, fn1) {
      ...
      if (num === 1) { // 单选
        ...
        if (num === 0) {
          ...
          const len = this.hoverEndDate.length
    ...
    hoverClass (date) {
      const $day = this.$refs.datePackerDay.querySelectorAll('.dataMouseoverAct')
      if (date.length === 2) {
        for (let i = 0, len = $day.length; i < len; i++) {
          const o = $day[i]
          const toDate = moment(o.getAttribute('data-mouseover')).format('YYYYMMDD')
          const startDate = moment(new Date(date[0])).format('YYYYMMDD')
          const endDate = moment(new Date(date[1])).format('YYYYMMDD')
    ...
    // 监听切换年月
    ymChange (date, ref) {
      const index = ref.getAttribute('index') - 0; let value = date; let decrease = date
      for (let i = index + 1; i < this.panelNum; i++) {
        const vDateRange = this.$refs['dateRange' + i][0]
